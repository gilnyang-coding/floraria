...
/// <summary>
/// 은 활 아이템 - 좌클릭 시 가장 가까운 적에게 화살 발사
/// - WoodBow보다 강력한 스탯
/// - 시속 160km (44.44 m/s)로 화살이 날아감
/// - 히트스캔 방식으로 무조건 맞음
/// - BaseItem 상속으로 월드 드롭/줍기 지원
/// </summary>
public class SilverBowItem : BaseItem, IWeapon {
    [Header("Bow Settings")]
    [SerializeField] private float damage = 40f; // WoodBow(25)보다 강함
    [SerializeField] private float range = 25f; // 최대 사거리 (WoodBow 20보다 길음)
    ...
    [Header("Cooldown")]
    [SerializeField] private float cooldown = 0.8f; // WoodBow(1.0)보다 빠름
    private float lastAttackTime = -999f;
    
    public void Attack(GameObject user) {
        // 쿨타임 체크
        if (Time.time - lastAttackTime < cooldown) {
            Debug.Log($"[SilverBowItem] 쿨타임 중... ({cooldown - (Time.time - lastAttackTime):F1}초 남음)");
            return;
        }
        ...
        if (closestEnemy == null) {
            Debug.Log("[SilverBowItem] 범위 내에 적이 없습니다.");
            return;
        }
        ...
        Debug.Log($"[SilverBowItem] {closestEnemy.name}에게 화살 발사!");
    }
    ...
    private IEnumerator ShootArrow(GameObject user, Transform target) {
        ...
        if (target != null) {
            ...
            if (damageable != null) {
                ...
                Debug.Log($"[SilverBowItem] {target.name}에게 {damage} 데미지!");
            }
        }
    }
