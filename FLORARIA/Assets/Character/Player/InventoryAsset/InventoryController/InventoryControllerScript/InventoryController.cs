...
public void AllignDictionaries() {
    inventoryManager.Clear();
    EnableDisableDict.Clear(); // <-- 중요: 기존의 잘못된 정보를 싹 비웁니다.
    inventoryUIDict.Clear();

    foreach (GameObject InventoryUI in allInventoryUI) {
        if (InventoryUI == null) continue;

        InventoryUIManager inventoryInstance = InventoryUI.GetComponent<InventoryUIManager>();
        string invName = inventoryInstance.GetInventoryName();

        if (!inventoryUIDict.ContainsKey(invName)) {
            inventoryUIDict.Add(invName, InventoryUI);
        }

        inventoryInstance.GetInventory().InitList();
        inventoryManager.Add(invName, inventoryInstance.GetInventory());

        // 'char'에서 'string'으로 바뀐 리스트를 딕셔너리에 등록합니다.
        foreach (string key in inventoryInstance.GetEnableDisable()) {
            string lowerKey = key.ToLower().Trim(); // 공백 제거 및 소문자화
            if (!EnableDisableDict.ContainsKey(lowerKey)) {
                EnableDisableDict.Add(lowerKey, new List<GameObject>());
            }
            EnableDisableDict[lowerKey].Add(InventoryUI);
            Debug.Log($"[등록 완료] {lowerKey} 키에 {invName} 인벤토리가 연결됨");
        }
    }
}
...
private void ExecuteKeyAction(string key, bool canOpen) {
    if (EnableDisableDict.ContainsKey(key)) {
        Debug.Log($"{key} 키에 등록된 인벤토리 개수: {EnableDisableDict[key].Count}"); // <-- 로그 추가
        foreach (GameObject inventoryUI in EnableDisableDict[key]) {
            if (inventoryUI.activeSelf) {
                inventoryUI.SetActive(false);
                Debug.Log($"{key} 키로 인벤토리 닫음");
            }
            else if (canOpen) {
                inventoryUI.SetActive(true);
                Debug.Log($"{key} 키로 인벤토리 켬");
            }
        }
    } else {
        Debug.LogWarning($"{key} 키가 딕셔너리에 등록되지 않았습니다!"); // <-- 로그 추가
    }
}
